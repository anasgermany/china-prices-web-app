...
class AppProvider extends ChangeNotifier {
    ...
    try {
      ...
      for (String jsonUrl in jsonUrls) {
        try {
          ...
          print('AppProvider: Loaded ${products.length} products from $jsonUrl');
      ...
      print('AppProvider: Total products loaded and shuffled: ${_products.length}');
    ...
    try {
      ...
      for (String jsonUrl in jsonUrls) {
        try {
          ...
          final products = await apiService.fetchProducts(jsonUrl);
          print('AppProvider: Loaded ${products.length} fresh products from $jsonUrl');
          allProducts.addAll(products);
        } catch (e) {
          print('AppProvider: Error loading from $jsonUrl: $e');
        }
      ...
      print('AppProvider: Total fresh products loaded from all JSONs: ${allProducts.length}');
      ...
      print('AppProvider: Found and shuffled ${_searchResults.length} results for "$query"');
  ...
  Future<void> loadProductsByCategory(String category) async {
    try {
      ...
      print('Total fresh and shuffled products loaded for category "$category": ${categoryProducts.length}');
  ...
  Future<void> addToFavorites(Product product) async {
    try {
      print('AppProvider: Adding product to favorites: ${product.productDesc} (ID: ${product.productId})');
      
      // Check if already in favorites
      if (!_favorites.any((p) => p.productId == product.productId)) {
        _favorites.add(product);
        print('AppProvider: Successfully added to favorites, total: ${_favorites.length}');
        notifyListeners();
      } else {
